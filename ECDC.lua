...
function ECDC:Unlock()
	ECDC_Button:Show()
	for i=1,10 do
		ECDC_ToolTips[i] = 'test'..i
		ECDC_ToolTipDetails[i] = 'test'..i
...
function ECDC_OnUpdate()
	if ECDC_Locked then
		local i = 1

		local temp = {}
		sort(ECDC_UsedSkills, function(a, b) local ta, tb = a.started + a.countdown - GetTime(), b.started + b.countdown - GetTime() return ta < tb or tb == ta and a.skill < b.skill end)
		for k, v in ECDC_UsedSkills do
			...
			if timeleft > 0 then
				...
				-- Warrior enrage isnt a CD, Druid Enrage is!
				if v.player == UnitName('target') and i <= 10 and not (UnitClass('target') == 'Warrior' and v.skill == 'Enrage') and ECDC_ToolTips[i-1] ~= v.skill then
					if timeleft <= 5 then
						getglobal('ECDC_Tex'..i):SetAlpha(1 - (math.sin(GetTime() * 1.3 * math.pi) + 1) / 2 * .7)
					else
						getglobal('ECDC_Tex'..i):SetAlpha(1)
					end
...
function ECDC_LoadSkills()
	...
	-- Trinkets & Racials
		{name = "Will of the Forsaken", cooldown = 2*60, desc = "Unknown!", icon = "Spell_Shadow_RaiseDead"},
		{name = "Perception", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Nature_Sleep"},
		{name = "War Stomp", cooldown = 2*60, desc = "Unknown!", icon = "Ability_WarStomp"},
		{name = "Stoneform", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Shadow_UnholyStrength"},

		{name = "Brittle Armor", cooldown = 2*60, desc = "Unknown!", icon = "Spell_Shadow_GrimWard"},
		{name = "Unstable Power", cooldown = 2*60, desc = "Unknown!", icon = "Spell_Lightning_LightningBolt01"},
		{name = "Restless Strength", cooldown = 2*60, desc = "Unknown!", icon = "Spell_Shadow_GrimWard"},
		{name = "Ephemeral Power", cooldown = 90, desc = "Unknown!", icon = "Spell_Holy_MindVision"},
		{name = "Massive Destruction", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Fire_WindsofWoe"},
		{name = "Arcane Potency", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Arcane_StarFire"},
		{name = "Energized Shield", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Nature_CallStorm"},
		{name = "Brilliant Light", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Holy_MindVision"},
		{name = "Mar'li's Brain Boost", cooldown = 3*60, desc = "Unknown!", icon = "INV_ZulGurubTrinket"},
		{name = "Gift of Life", cooldown = 5*60, desc = "Unknown!", icon = "INV_Misc_Gem_Pearl_05"},
		{name = "Nature Aligned", cooldown = 5*60, desc = "Unknown!", icon = "Spell_Nature_SpiritArmor"},
		{name = "Earthstrike", cooldown = 2*60, desc = "Unknown!", icon = "Spell_Nature_AbolishMagic"},

		{name = "Frost Recleftor", cooldown = 5*60, desc = "Unknown!", icon = "Spell_Frost_FrostWard"},
		{name = "Shadow Reflector", cooldown = 5*60, desc = "Unknown!", icon = "Spell_Shadow_AntiShadow"},
		{name = "Fire Reflector", cooldown = 5*60, desc = "Unknown!", icon = "Spell_Fire_SealOfFire"},
		...
		{name = "Mocking Blow", cooldown = 2*60, desc = "A mocking attack that causes 93 damage, a moderate amount of threat and forces the target to focus attacks on you for 6 sec.", icon = "Ability_Warrior_PunishingBlow"},
		...
		{name = "Challenging Shout", cooldown = 10*60, desc = "Forces all nearby enemies to focus attacks on you for 6 sec.", icon = "Ability_BullRush"},
		...
		{name = "Intimidating Shout", cooldown = 3*60, desc = "The warrior shouts, causing the targeted enemy to cower in fear. Up to 5 total nearby enemies will flee in fear. Lasts 8 sec.", icon = "Ability_GolemThunderClap"},
		...
		{name = "Last Stand", cooldown = 10*60, desc = "When activated, this ability temporarily grants you 30% of your maximum hit points for 20 seconds. After the effect expires, the hit points are lost.", icon = "Spell_Holy_AshesToAshes"},
		{name = "Death Wish", cooldown = 3*60, desc = "When activated, increases your physical damage by 20% and makes you immune to Fear effects, but lowers your armor and all resistances by -20%. Lasts 30 sec.", icon = "Spell_Shadow_DeathPact"},
		...
		{name = "Divine Favor", cooldown = 2*60, desc = "Unknown!", icon = "Spell_Holy_Heal"},
		{name = "Holy Shock", cooldown = 30, desc = "Unknown!", icon = "Spell_Holy_SearingLight"},
		{name = "Holy Shield", cooldown = 10, desc = "Unknown!", icon = "Spell_Holy_BlessingOfProtection"},
		...
		{name = "Elemental Mastery", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Nature_WispHeal"},
		{name = "Stormstrike", cooldown = 20, desc = "Unknown!", icon = "Spell_Holy_SealOfMight"},
		...
		{name = "Wyvern Sting", cooldown = 2*60, desc = "A stinging shot that puts the target to sleep for 12 sec. Any damage will cancel the effect. When the target wakes up, the Sting causes 600 Nature damage over 12 sec. Only usable out of combat. Only one Sting per Hunter can be active on the target at a time.", icon = "INV_Spear_02"},

		{name = "Bestial Wrath", cooldown = 2*60, desc = "Unknown!", icon = "Ability_Druid_FerociousBite"},
		{name = "Intimidation", cooldown = 60, desc = "Unknown!", icon = "Ability_Devour"},
		{name = "Detterance", cooldown = 5*60, desc = "Unknown!", icon = "Ability_Whirlwind"},
		{name = "Scattershot", cooldown = 30, desc = "Unknown!", icon = "Ability_GolemStormBolt"},
		...
		{name = "Death Coil", cooldown = 2*60, desc = "Causes the enemy target to run in horror for 3 sec and causes 470 Shadow damage. The caster gains 100% of the damage caused in health.", icon = "Spell_Shadow_DeathCoil"},
		...
		{name = "Fel Domination", cooldown = 15*60, desc = "Unknown!", icon = "Spell_Nature_RemoveCurse"},
		...
		{name = "Desperate Prayer", cooldown = 10*60, desc = "Instantly heals the caster for 1324 to 1562.", icon = "Spell_Holy_Restoration"},
		...
		{name = "Devouring Plague", cooldown = 3*60, desc = "Afflicts the target with a disease that causes 904 Shadow damage over 24 sec. Damage caused by the Devouring Plague heals the caster.", icon = "Spell_Shadow_BlackPlague"},
		...
		{name = "Inner Focus", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Frost_WindWalkOn"},
		{name = "Power Infusion", cooldown = 3*60, desc = "Unknown!", icon = "Spell_Holy_PowerInfusion"},
		{name = "Silence", cooldown = 45, desc = "Unknown!", icon = "Spell_Shadow_ImpPhaseShift"},
		...
		{name = "Challenging Roar", cooldown = 10*60, desc = "Forces all nearby enemies to focus attacks on you for 6 sec.", icon = "Ability_Druid_ChallangingRoar"},
		...
		{name = "Dash", cooldown = 5*60, desc = "Increases movement speed by 60% for 15 sec. Does not break prowling.", icon = "Ability_Druid_Dash"},
		...
		{name = "Frenzied Regeneration", cooldown = 3*60, desc = "Converts up to 10 rage per second into health for 10 sec. Each point of rage is converted into 20 health.", icon = "Ability_BullRush"},
		...
		{name = "Innervate", cooldown = 6*60, desc = "Increases the target's Mana regeneration by 400% and allows 100% of the target's Mana regeneration to continue while casting. Lasts 20 sec.", icon = "Spell_Nature_Lightning"},
